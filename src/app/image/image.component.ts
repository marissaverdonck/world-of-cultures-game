...
export class ImageComponent {
  ...
  // Wanneer er een goed antwoord is gegeven gaat de counter lopen. 
  countGood() {
    this.count++ ;
...
console.log("gefeliciteerd")}
      ...
      // Vervolgstap: naar nieuwe categorie
    }}
    
// Bron: hulp van Mohamad en Patrick
// inject HttpClient into the constructor in a private property 
  constructor(private http: HttpClient) {
      // Krijg data binnen via http.get en voer de functie uit: response1
      this.http.get(this.url1).subscribe(response1=> {
          const responseData = response1;
          //Haal uit de data de results.bindings
          this.data = response1.results.bindings;
          //Haal een random image op uit de binnengehaalde data
          let randomImage = Math.floor(Math.random() * Math.floor(response1.results.bindings.length))
          //Plaats random afbeelding in afbeeldingslink
